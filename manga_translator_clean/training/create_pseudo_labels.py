...
def parse_args():
    ...
    parser.add_argument('--enable-ocr',         action='store_true', help='Run OCR on detected boxes and store text summaries in the review manifest')
    parser.add_argument('--ocr-model-id',       default='manga-ocr', help='Display name stored in the manifest for the OCR model used')
    parser.add_argument('--manifest-name',      default='review_manifest.json', help='Filename for the generated review manifest')
...
def summarize_ocr_text(ocr_model, image_path: Path, boxes):
    if ocr_model is None or not boxes:
        return ""

    try:
        image = Image.open(image_path).convert("RGB")
    except OSError as exc:
        logging.warning(f"Skipping OCR summary for {image_path}: {exc}")
        return ""

    texts = []
    for _cls, x1, y1, x2, y2, _conf in boxes:
        crop = image.crop((int(x1), int(y1), int(x2), int(y2)))
        text = ocr_model.extract_text(crop).strip()
        if text:
            texts.append(text)
...
def main():
    ...
    if args.enable_ocr:
        try:
            ...
            logging.info("OCR summaries enabled for pseudo-label review manifest")
        except Exception as exc:
            logging.warning(f"OCR model could not be loaded, continuing without OCR summaries: {exc}")
            ocr_model = None
    ...
    manifest = {
        ...
        "detector_model": str(Path(args.model).resolve()),
        "ocr_model": args.ocr_model_id if ocr_model else None,
        "settings": {
            "conf": args.conf,
            "iou": args.iou,
            "imgsz": args.imgsz,
    ...
    for start in range(0, len(images), chunk):
        ...
        for res in results:
            ...
            manifest["items"].append({
                ...
                "overlay_path": str(vis_file.resolve()) if vis_file else None,
                "detector_model": str(Path(args.model).resolve()),
                "ocr_model": args.ocr_model_id if ocr_model else None,
                "detection_count": len(norm_boxes),
                "ocr_summary": summarize_ocr_text(ocr_model, img_path, vis_boxes),
